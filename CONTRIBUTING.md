# Contributing to Jue Agent

Thank you for contributing to Jue Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Jue to work everywhere.
...
Bundled skills (in `skills/`) ship with every Jue install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `jue skills browse` (labeled "official") and install it with `jue skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `jue skills install`.
...
```bash
mkdir -p ~/.jue/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.jue/config.yaml
touch ~/.jue/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.jue/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/jue" ~/.local/bin/jue
...
├── cli.py                    # JueCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (jue-cli, jue-telegram, etc.)
├── jue_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── skills/                   # Bundled skills (copied to ~/.jue/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (jue-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.jue/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.jue/`)

| Path | Purpose |
|------|---------|
| `~/.jue/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.jue/.env` | API keys and secrets |
| `~/.jue/auth.json` | OAuth credentials (Nous Portal) |
| `~/.jue/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.jue/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.jue/state.db` | SQLite session database |
| `~/.jue/sessions/` | JSON session logs |
| `~/.jue/cron/` | Scheduled job data |
| `~/.jue/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`jue_state.py`) with full-text search and unique session titles. JSON logs go to `~/.jue/sessions/`.
...
Four fields are supported under `metadata.jue`:
...
The user may skip setup and keep loading the skill. Jue only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `jue setup` or update `~/.jue/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Jue tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `jue --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Jue uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.jue/skins/<name>.yaml`:
...
Jue runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Jue has terminal access. Security matters.
...
2. **Test manually**: Run `jue` and exercise the code path you changed
...
- Include: OS, Python version, Jue version (`jue version`), full error traceback
