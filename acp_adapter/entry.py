"""CLI entry point for the jue-agent ACP adapter.

Loads environment variables from ``~/.jue/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from JUE_HOME (default ``~/.jue``)."""
    from hermes_cli.env_loader import load_hermes_dotenv

    jue_home = get_jue_home()
    loaded = load_hermes_dotenv(jue_home=jue_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", jue_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting jue-agent ACP adapter")
    ...
    import acp
    from .server import JueACPAgent
