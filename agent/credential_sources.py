"""Unified removal contract for every credential source Jue reads from.

Jue seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.jue/.env
    claude_code   — ~/.claude/.credentials.json
    jue_pkce   — ~/.jue/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``jue auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), jue_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.jue/.env  → clear it
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Jue.",
            f"  The pool entry is now suppressed — Jue will ignore "
            f"{env_var} until you run `jue auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Jue stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `jue auth add anthropic` to re-enable if needed.",
    ])


def _remove_jue_pkce(provider: str, removed) -> RemovalResult:
    """~/.jue/.anthropic_oauth.json is ours — delete it outright."""
    from jue_constants import get_jue_home

    result = RemovalResult()
    oauth_file = get_jue_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Jue Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `jue login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `jue auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Jue auth store is not enough — _seed_from_singletons() would
    ...
    via ``jue auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `jue auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `jue auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Jue stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `jue auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="jue_pkce",
        remove_fn=_remove_jue_pkce,
        description="~/.jue/.anthropic_oauth.json",
    ))
    register(RemovalStep(
