...
as ``kind: backend``) or ``~/.jue/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``jue tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``jue tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``jue tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$JUE_HOME/cache/images/``, creating parents as needed."""
    from jue_constants import get_jue_home

    path = get_jue_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$JUE_HOME/cache/images/``.
