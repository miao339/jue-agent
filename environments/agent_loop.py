"""
JueAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the jue-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to jue-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by JueAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by JueAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class JueAgentLoop:
    """
    Runs jue-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # jue-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("jue")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via jue-agent's dispatch
