...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs JueAgentLoop with terminal tool only
...
Requires: pip install jue-agent[yc-bench]
"""
...
class YCBenchEvalConfig(JueAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends JueAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(JueAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The JueAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs JueAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our JueAgentLoop.
