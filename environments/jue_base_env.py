"""
JueAgentBaseEnv -- Abstract Base Environment for Jue-Agent + Atropos

Provides the Atropos integration plumbing that all jue-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via JueAgentLoop
...
# Ensure the jue-agent repo root is on sys.path so that imports like
...
# Load API keys from jue-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, JueAgentLoop
...
# Import jue-agent toolset infrastructure
...
class JueAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for jue-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of jue toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: jue, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/jue-results/ "
...
class JueAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for jue-agent Atropos integration.
    ...
    name: Optional[str] = "jue-agent"
    env_config_cls = JueAgentEnvConfig

    def __init__(
        self,
        config: JueAgentEnvConfig,
        ...
        # Set terminal environment variables so jue tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY jue-agent tool (terminal, file, web,
