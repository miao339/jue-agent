...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Jue Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Jue Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Jue Agent",
    },
    {
...
class TerminalTestEnv(JueAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/jue-agent/.env.
        """
