...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.jue/.env manually."
)
...
# Default location: {JUE_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_jue_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_jue_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_jue_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_jue_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          JUE_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          JUE_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          JUE_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("JUE_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("JUE_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("JUE_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        callback_generation = getattr(interrupt_event, "_jue_run_generation", None)
