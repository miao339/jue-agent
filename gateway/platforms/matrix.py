...
# Uses get_jue_home() so each profile gets its own Matrix store.
from jue_constants import get_jue_dir as _get_jue_dir

_STORE_DIR = _get_jue_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("JUE_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("JUE_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        mangle file paths like ``/home/jue/media/file.png``.
        """
        if self._user_id:
