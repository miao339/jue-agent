# Jue Agent Has Had "Routines" Since March
...
Jue equivalent — works today:
```bash
jue webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Jue implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Jue Agent |
|---|---|---|
...
## Things Jue Does That Routines Can't
...
Jue has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Jue Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install jue-agent
jue setup
```
...
```bash
jue gateway setup    # enable webhooks
jue webhook subscribe pr-review \
  --events "pull_request" \
...
*Jue Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
