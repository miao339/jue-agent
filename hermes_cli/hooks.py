"""jue hooks — inspect and manage shell-script hooks.

Usage::

    jue hooks list
    jue hooks test <event> [--for-tool X] [--payload-file F]
    jue hooks revoke <command>
    jue hooks doctor

Consent records live under ``~/.jue/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.jue/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``jue hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: jue hooks {list|test|revoke|doctor}")
        print("Run 'jue hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.jue/config.yaml.")
        print("See `jue hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `jue hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `jue hooks test` and `jue hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Jue wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `jue hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `jue hooks doctor` would execute
