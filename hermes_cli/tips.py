...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or jue -c.",
    ...
    "Ctrl+Z suspends Jue to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "jue -c resumes your most recent CLI session. jue -c \"project name\" resumes by title.",
    "jue -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "jue -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "jue chat -t web,terminal enables only specific toolsets for a focused session.",
    "jue chat -s github-pr-workflow preloads a skill at launch.",
    "jue chat -q \"query\" runs a single non-interactive query and exits.",
    "jue chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "jue chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "jue --yolo bypasses all dangerous command approval prompts for the entire session.",
    "jue chat --source telegram tags the session for filtering in jue sessions list.",
    "jue -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "jue doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "jue dump outputs a compact setup summary — great for bug reports.",
    "jue config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "jue config edit opens config.yaml in your default editor.",
    "jue config check scans for missing or stale configuration options.",
    "jue sessions browse opens an interactive session picker with search.",
    "jue sessions stats shows session counts by platform and database size.",
    "jue sessions prune --older-than 30 cleans up old sessions.",
    "jue skills search react --source skills-sh searches the skills.sh public directory.",
    "jue skills check scans installed hub skills for upstream updates.",
    "jue skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "jue skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "jue mcp add github --command npx adds MCP servers from the command line.",
    "jue mcp serve runs Jue itself as an MCP server for other agents.",
    "jue auth add lets you add multiple API keys for credential pool rotation.",
    "jue completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "jue logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "jue backup creates a zip backup of your entire Jue home directory.",
    "jue profile create coder creates an isolated profile that becomes its own command.",
    "jue profile create work --clone copies your current config and keys to a new profile.",
    "jue update syncs new bundled skills to ALL profiles automatically.",
    "jue gateway install sets up Jue as a system service (systemd/launchd).",
    "jue memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "jue webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as jue -w).",
    ...
    "JUE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Jue tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'jue profile create coder' creates the 'coder' command.",
    "jue profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Jue prints a resume command with session ID and stats.",
    "jue sessions export backup.jsonl exports all sessions for backup or analysis.",
    "jue -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "jue skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .jue/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: jue cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Jue runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "jue gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Jue via a local macOS server.",
    ...
    "Context files (.jue.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.jue/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Jue loads project context from .jue.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — jue tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.jue/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "jue chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "jue plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "jue acp runs Jue as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "JUE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "jue login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "jue status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.jue/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.jue/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Jue your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.jue/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Jue auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.jue/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at JUE_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "JUE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in JUE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.jue/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'jue profile create ls' would be rejected.",
    "jue profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "jue claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "JUE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via jue tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.jue/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.jue/skills/.hub/quarantine/ holds skills pending security review.",
]
