"""
Unified tool configuration for Jue Agent.

`jue tools` and `jue setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.jue/config.yaml under
...
# They're still in _JUE_CORE_TOOLS (available at runtime if enabled),
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_jue_home()}/jue-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in jue-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "jue-cli" (which include all _JUE_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "jue-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `jue tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by jue tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (jue-cli, jue-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `jue tools` and `jue setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Jue Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://jue-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from jue_constants import display_jue_home
    print(color(f"  Tool configuration saved to {display_jue_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'jue' or gateway restart.", Colors.DIM))
    print()
