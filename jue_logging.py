"""Centralized logging setup for Jue Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.jue/logs/`` (profile-aware via ``get_jue_home()``).
...
from jue_constants import get_config_path, get_jue_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_jue_session_injector", False):
    ...
    _session_record_factory._jue_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``jue logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the Jue logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    jue_home
        Override for the Jue home directory.  Falls back to
        ``get_jue_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_jue_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._jue_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the jue group. However, both _open() (initial creation) and
