"""
Jue MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Jue-specific extra)

Usage:
    jue mcp serve
    jue mcp serve --verbose
...
logger = logging.getLogger("jue.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using JUE_HOME."""
    return get_jue_home() / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    directory_file = get_jue_home() / "channel_directory.json"
...
class EventBridge:
    ...
    This is the Jue equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Jue MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "jue",
        instructions=(
            "Jue Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Jue MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
