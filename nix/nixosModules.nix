# nix/nixosModules.nix — NixOS module for jue-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: jue runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $JUE_HOME/.env
# and read by jue at startup — no container recreation needed for env changes.
#
# Tool resolution: the jue wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.jue-agent;
    jue-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-jue-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "jue-config-attrs";
      description = "Jue YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "jue-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "jue-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'JUE_DOC_EOF'\n${value}\nJUE_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "jue-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/jue";
    ...
    # jue user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "jue-container-entrypoint" ''
      set -eu

      JUE_UID="''${JUE_UID:?JUE_UID must be set}"
      JUE_GID="''${JUE_GID:?JUE_GID must be set}"

      # ── Group: ensure a group with GID=$JUE_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$JUE_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="jue"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$JUE_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$JUE_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$JUE_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$JUE_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="jue"
        TARGET_HOME="/home/jue"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$JUE_UID" -g "$JUE_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$JUE_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$JUE_UID:$JUE_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure JUE_HOME is owned by the target user
      if [ -n "''${JUE_HOME:-}" ] && [ -d "$JUE_HOME" ]; then
        chown -R "$JUE_UID:$JUE_GID" "$JUE_HOME"
      fi
      ...
      if [ ! -f /var/lib/jue-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/jue-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/jue ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/jue
        chmod 0440 /etc/sudoers.d/jue
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$JUE_UID" --regid="$JUE_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $JUE_HOME/.env.
    ...
    # Default: /var/lib/jue/workspace → /data/workspace.
  ...
  in {
    options.services.jue-agent = with lib; {
      enable = mkEnableOption "Jue Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = jue-agent;
        description = "The jue-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/jue";
        description = "State directory. Contains .jue/ subdir (JUE_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Jue config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $JUE_HOME/.env at activation time.
          Jue reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $JUE_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $JUE_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `jue gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the jue CLI to environment.systemPackages and export
          JUE_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.jue symlink to the service
            stateDir. These users are automatically added to the jue group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.jue-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the jue CLI to system PATH and export JUE_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.jue/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.JUE_HOME = "${cfg.stateDir}/.jue";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.jue-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed jue
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure jue is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.jue        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jue/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jue/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jue/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jue/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."jue-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.jue
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.jue ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.jue ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.jue -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.jue/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.jue/$_subdir"
            chmod 2770 "${cfg.stateDir}/.jue/$_subdir"
            find "${cfg.stateDir}/.jue/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.jue/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.jue/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jue/config.yaml
            chmod 0640 ${cfg.stateDir}/.jue/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.jue/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jue/.managed
          chmod 0644 ${cfg.stateDir}/.jue/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.jue/.container-mode <<'JUE_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
jue_bin=${containerDataDir}/current-package/bin/jue
JUE_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jue/.container-mode
            chmod 0644 ${cfg.stateDir}/.jue/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.jue/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jue";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.jue" ]; then
                  rm -f "${symlinkPath}"
                  echo "jue-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.jue -> stateDir/.jue for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jue";
                target = "${cfg.stateDir}/.jue";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "jue-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jue/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.jue/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jue/auth.json
              fi
            ''}
          ''}
          ...
          # Jue reads $JUE_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.jue/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'JUE_NIX_ENV_EOF'
${envFileContent}
JUE_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.jue-agent = {
          description = "Jue Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $JUE_HOME/.env by the activation script. load_hermes_dotenv()
            ...
            # so interactive users in the jue group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.jue-agent = {
          description = "Jue Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              JUE_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              JUE_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env JUE_UID="$JUE_UID" \
                --env JUE_GID="$JUE_GID" \
                --env JUE_HOME=${containerDataDir}/.jue \
                --env JUE_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/jue gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
