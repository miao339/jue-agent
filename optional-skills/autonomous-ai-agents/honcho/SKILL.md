---
name: honcho
description: Configure and use Honcho memory with Jue -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Jue Agent
license: MIT
metadata:
...
# Honcho Memory for Jue

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Jue profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/jue#running-honcho-locally-with-jue

### Verify

```bash
jue honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Jue profile's AI peer
...
Honcho models conversations as interactions between **peers**. Jue creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Jue instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Jue run |
| `global` | Single session across all directories |

Manual override: `jue honcho map my-project-name`
...
Each Jue profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block jue.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `jue.coder` host block in `honcho.json`
...
```bash
jue honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Jue when Honcho memory is active.
...
Config file: `$JUE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `jue honcho setup`. Ensure `memory.provider: honcho` is in `~/.jue/config.yaml`.

### Memory not persisting across sessions
Check `jue honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `jue profile create <name> --clone`. For existing profiles: `jue honcho sync`.
...
| Command | Description |
|---------|-------------|
| `jue honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `jue honcho status` | Show resolved config, connection test, peer info for active profile |
| `jue honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `jue honcho disable` | Disable Honcho for the active profile |
| `jue honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `jue honcho peers` | Show peer identities across all profiles |
| `jue honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `jue honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `jue honcho sessions` | List known directory-to-session-name mappings |
| `jue honcho map <name>` | Map current working directory to a Honcho session name |
| `jue honcho identity` | Seed AI peer identity or show both peer representations |
| `jue honcho sync` | Create host blocks for all Jue profiles that don't have one yet |
| `jue honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Jue + Honcho |
| `jue memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `jue memory status` | Show active memory provider and config |
| `jue memory off` | Disable external memory provider |
