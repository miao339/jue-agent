...
JUE_HOME_DIR="${JUE_HOME:-$HOME/.jue}"
JUE_CFG="${JUE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Jue config has twozero_td MCP entry ──
if [[ ! -f "$JUE_CFG" ]]; then
    echo -e " ${FAIL} Jue config not found at ${JUE_CFG}"
    manual_steps+=("Create ${JUE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$JUE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Jue config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Jue config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${JUE_CFG} manually"); }
    manual_steps+=("Restart Jue session to pick up config change")
fi
