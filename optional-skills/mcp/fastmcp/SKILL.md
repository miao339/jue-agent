...
- smoke-test a server with the FastMCP CLI before wiring it into Jue or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Jue. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.jue/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.jue/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Jue integration, either:

- configure the server in `~/.jue/config.yaml` using the `native-mcp` skill, or
...
### Jue cannot see the deployed server

The server-building part may be correct while the Jue config is not. Load the `native-mcp` skill and configure the server in `~/.jue/config.yaml`, then restart Jue.
