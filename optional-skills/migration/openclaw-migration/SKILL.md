...
description: Migrate a user's OpenClaw customization footprint into Jue Agent. Imports Jue-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Jue Agent (Nous Research)
license: MIT
metadata:
  jue:
    tags: [Migration, OpenClaw, Jue, Memory, Persona, Import]
    related_skills: [jue-agent]
---

# OpenClaw -> Jue Migration

Use this skill when a user wants to move their OpenClaw setup into Jue Agent with minimal manual cleanup.
...
```bash
jue claw migrate              # Full interactive migration
jue claw migrate --dry-run    # Preview what would be migrated
jue claw migrate --preset user-data   # Migrate without secrets
jue claw migrate --overwrite  # Overwrite existing conflicts
jue claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `jue setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_jue.py` to:

- import `SOUL.md` into the Jue home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Jue memory entries
- merge OpenClaw command approval patterns into Jue `command_allowlist`
- migrate Jue-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.jue/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Jue workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.jue/tts/`
- archive non-secret docs that do not have a direct Jue destination
...
- `scripts/openclaw_to_jue.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py`

Do not guess a shorter path like `~/.jue/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.jue/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Jue-compatible secrets, currently:
...
Jue CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Jue. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Jue workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Jue skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.jue/skills/migration/openclaw-migration/scripts/openclaw_to_jue.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Jue unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Jue targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Jue destination. Unsupported auth blobs must still be reported as skipped.
...
- Jue persona state imported
- Jue memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.jue/skills/openclaw-imports/`
