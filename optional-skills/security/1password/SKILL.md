...
version: 1.0.0
author: arceus77-7, enhanced by Jue Agent
license: MIT
metadata:
  jue:
...
- `tmux` available for stable authenticated sessions during Jue terminal calls (desktop app flow only)
...
### Service Account (recommended for Jue)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.jue/.env` (the skill will prompt for this on first load).
...
## Jue Execution Pattern (desktop app flow)

Jue terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/jue-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/jue-op.sock"
