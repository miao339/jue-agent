...
## Token Lifecycle
- Store at `~/.jue/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
- OR: accept user-provided client_id via JUE_GEMINI_CLIENT_ID env var
...
- jue-agent Copilot OAuth: `hermes_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
