"""disk_cleanup — ephemeral file cleanup for Jue Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under JUE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly JUE_HOME and /tmp/jue-*
Never touches: ~/.jue/logs/ or any system directory.
"""
...
try:
    ...
    def get_jue_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("JUE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".jue").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$JUE_HOME/logs/``."""
    return get_jue_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$JUE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under JUE_HOME or ``/tmp/jue-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    jue_home = get_jue_home()
    try:
        path.resolve().relative_to(jue_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/jue-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("jue-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside JUE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under JUE_HOME (but leave JUE_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "jue-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(jue_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == jue_home:
                continue
            try:
                rel_parts = dirpath.relative_to(jue_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    jue_home = get_jue_home()
    try:
        rel = path.resolve().relative_to(jue_home)
        ...
        # Path isn't under JUE_HOME (e.g. /tmp/jue-*) — fall through.
        pass
