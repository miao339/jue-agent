...
Daemon startup logs: `~/.jue/logs/hindsight-embed.log`
...
Config file: `~/.jue/hindsight/config.json`
...
| `retain_context` | `conversation between Jue Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.jue/.env` as `HINDSIGHT_LLM_API_KEY`.
