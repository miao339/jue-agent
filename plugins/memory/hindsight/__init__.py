...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: jue)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $JUE_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $JUE_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_jue_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "jue": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "jue"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Jue Agent and the User"
    ...
    def save_config(self, values, jue_home):
        """Write config to $JUE_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(jue_home) / "hindsight"
    ...
    def post_setup(self, jue_home: str, config: dict) -> None:
        ...
        provider_config["bank_id"] = "jue"
        provider_config["recall_budget"] = "mid"
        bank_id = "jue"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, jue_home)

        if env_writes:
            env_path = Path(jue_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "jue"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Jue Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "jue"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "jue"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("jue", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "jue")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Jue Agent and the User")
