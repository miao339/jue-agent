...
def _load_supermemory_config(jue_home: str) -> dict:
    config = _default_config()
    config_path = Path(jue_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, jue_home: str) -> None:
    config_path = Path(jue_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `jue memory setup`.
        # All other options are documented for $JUE_HOME/supermemory.json
    ...
    def save_config(self, values, jue_home):
        ...
        _save_supermemory_config(sanitized, jue_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from jue_constants import get_jue_home
        self._jue_home = kwargs.get("jue_home") or str(get_jue_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._jue_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "jue", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "jue_memory", "target": target, "type": "explicit_memory"},
