...
AUTHOR_MAP = {
    ...
    "mark.ramsell@juemounts.com": "mark-ramsell",
    ...
    "jue@nousresearch.com": "NousResearch",
    "jue@noushq.ai": "benbarclay",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "jue@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/miao339/jue-agent",
    ...
    lines.append(f"# Jue Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Jue Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Jue Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Jue Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Jue Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Jue Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Jue Agent v{new_version} ({calver_date})' "
