...
# 5. Symlinks the 'jue' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}Jue Agent Setup${NC}"
echo ""
...
# ============================================================================
# JUE_HOME runtime skeleton
# ============================================================================

echo -e "${CYAN}→${NC} Initializing JUE_HOME runtime layout..."
"$SETUP_PYTHON" - <<'PY'
from hermes_cli.config import ensure_jue_home

ensure_jue_home()
PY
echo -e "${GREEN}✓${NC} JUE_HOME initialized at ${JUE_HOME:-$HOME/.jue}"

# ============================================================================
# PATH setup — symlink jue into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up jue command..."

JUE_BIN="$SCRIPT_DIR/venv/bin/jue"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$JUE_BIN" "$COMMAND_LINK_DIR/jue"
echo -e "${GREEN}✓${NC} Symlinked jue → $COMMAND_LINK_DISPLAY_DIR/jue"
...
# ============================================================================
# Seed bundled skills into ~/.jue/skills/
# ============================================================================

JUE_SKILLS_DIR="${JUE_HOME:-$HOME/.jue}/skills"
mkdir -p "$JUE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.jue/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$JUE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  jue status        # Check configuration"
if is_termux; then
    echo "  jue gateway       # Run gateway in foreground"
else
    echo "  jue gateway install # Install gateway service (messaging + cron)"
fi
echo "  jue cron list     # View scheduled jobs"
echo "  jue doctor        # Diagnose issues"
echo ""
