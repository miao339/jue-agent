...
- Give it a title like "jue-agent-<machine-name>"
...
elif [ -f ~/.jue/.env ] && grep -q "^GITHUB_TOKEN=" ~/.jue/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.jue/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
