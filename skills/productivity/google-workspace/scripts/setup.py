#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Jue Agent.
...
try:
    from jue_constants import display_jue_home, get_jue_home
except ModuleNotFoundError:
    JUE_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if JUE_AGENT_ROOT.exists():
        sys.path.insert(0, str(JUE_AGENT_ROOT))
    from jue_constants import display_jue_home, get_jue_home

JUE_HOME = get_jue_home()
TOKEN_PATH = JUE_HOME / "google_token.json"
CLIENT_SECRET_PATH = JUE_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = JUE_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Jue profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Jue home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_jue_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Jue")
