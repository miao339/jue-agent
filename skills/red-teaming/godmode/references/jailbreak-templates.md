...
## 5. GODMODE FAST — Jue 4 405B (Zero Refusal)

**Model:** `nousresearch/jue-4-405b`
**Strategy:** Jue models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Jue Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.jue/config.yaml`:
