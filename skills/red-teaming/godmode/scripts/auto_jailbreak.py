...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("JUE_HOME", os.path.expanduser("~/.jue")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("JUE_HOME", Path.home() / ".jue")) / "skills" / "red-teaming" / "godmode"
...
JUE_HOME = Path(os.getenv("JUE_HOME", Path.home() / ".jue"))
CONFIG_PATH = JUE_HOME / "config.yaml"
PREFILL_PATH = JUE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Jue models — already uncensored, just needs clean prompt
    "jue": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "jue" in model_lower or "nous" in model_lower:
        return "jue"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Jue config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.jue/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Jue for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Jue for changes to take effect.")
