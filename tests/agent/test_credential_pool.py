...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "jue" / "auth.json").read_text())
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_jue_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:jue_pkce", "jue_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JUE_HOME", str(tmp_path / "jue"))
    (tmp_path / "jue").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "jue" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
