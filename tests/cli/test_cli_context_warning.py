...
def _isolate(tmp_path, monkeypatch):
    """Isolate JUE_HOME so tests don't touch real config."""
    home = tmp_path / ".jue"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal JueCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = JueCLI.__new__(JueCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["jue-core"]
        obj.compact = False
