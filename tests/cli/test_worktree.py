...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("jue/jue-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned jue/* and pr-* branches."""

    def test_prunes_orphaned_jue_branch(self, git_repo):
        """jue/jue-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "jue/jue-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "jue/jue-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "jue/jue-deadbeef" in result.stdout
        ...
        assert "jue/jue-deadbeef" in orphaned
        ...
        assert "jue/jue-deadbeef" not in result.stdout
