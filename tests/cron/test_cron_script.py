...
- Path resolution (absolute, relative to JUE_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp JUE_HOME."""
    jue_home = tmp_path / ".jue"
    jue_home.mkdir()
    (jue_home / "cron").mkdir()
    (jue_home / "cron" / "output").mkdir()
    (jue_home / "scripts").mkdir()
    monkeypatch.setenv("JUE_HOME", str(jue_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "JUE_DIR", jue_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", jue_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", jue_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", jue_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("JUE_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.jue/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("JUE_SESSION_PLATFORM") is None
        assert os.environ.get("JUE_SESSION_CHAT_ID") is None
        assert os.environ.get("JUE_SESSION_CHAT_NAME") is None
