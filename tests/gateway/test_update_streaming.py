...
Tests the new --gateway mode for jue update, including:
...
def _make_runner(jue_home=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.3)
            (jue_home / ".update_response").write_text("y")
        ...
        assert not (jue_home / ".update_prompt.json").exists()
        assert not (jue_home / ".update_response").exists()
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = jue_home / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (jue_home / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
    ...
    def test_empty_response_returns_default(self, tmp_path):
        ...
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        (jue_home / ".update_response").write_text("")
...
class TestUpdateCommandGatewayFlag:
    """Verify the gateway spawns jue update --gateway."""
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        # Write output
        (jue_home / ".update_output.txt").write_text("→ Fetching updates...\n")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (jue_home / ".update_pending.json").write_text(json.dumps(pending))
        (jue_home / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (jue_home / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.5)
            (jue_home / ".update_response").write_text("y")
            (jue_home / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.3)
            (jue_home / ".update_exit_code").write_text("0")
    ...
    async def test_failure_exit_code(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (jue_home / ".update_pending.json").write_text(json.dumps(pending))
        (jue_home / ".update_output.txt").write_text("error occurred\n")
...
class TestUpdatePromptInterception:
    ...
    async def test_intercepts_response_when_prompt_pending(self, tmp_path):
        ...
        response_path = jue_home / ".update_response"
