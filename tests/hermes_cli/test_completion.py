...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real jue structure."""
    p = argparse.ArgumentParser(prog="jue")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_jue_completion()" in out
        assert "complete -F _jue_completion jue" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c jue -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_jue_profiles()" in out
        assert 'profiles_dir="$HOME/.jue/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'jue profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _jue_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_jue_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _jue_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_jue_profiles()" in out
        assert "$HOME/.jue/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__jue_profiles" in out
        count = out.count("(__jue_profiles)")
