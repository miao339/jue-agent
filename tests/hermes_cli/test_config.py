...
class TestGetJueHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JUE_HOME", None)
            home = get_hermes_home()
            assert home == Path.home() / ".jue"

    def test_env_override(self):
        with patch.dict(os.environ, {"JUE_HOME": "/custom/path"}):
            home = get_hermes_home()
            assert home == Path("/custom/path")


class TestEnsureJueHome:
    ...
    def test_creates_full_jue_runtime_layout(self, tmp_path):
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path)}):
            ensure_jue_home()
            ...
            assert (tmp_path / "harness3").is_dir()
            assert (tmp_path / "harness3" / "triplets").is_dir()
            assert (tmp_path / "harness3" / "harnesses").is_dir()

    def test_default_config_uses_xinxue_personality_and_streaming(self, tmp_path):
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path)}):
            ensure_jue_home()

        config = yaml.safe_load((tmp_path / "config.yaml").read_text(encoding="utf-8"))
        assert config["display"]["personality"] == "xinxue"
        assert config["display"]["streaming"] is True
        assert config["agent"]["personalities"]["xinxue"] == (
    ...
    def test_default_soul_md_comes_from_jue_harness2(self, tmp_path):
        expected = (
        ...
        assert (tmp_path / "SOUL.md").read_text(encoding="utf-8") == expected
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JUE_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
