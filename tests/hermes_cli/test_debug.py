"""Tests for ``jue debug`` CLI command and debug utilities."""
...
def jue_home(tmp_path, monkeypatch):
    """Set up an isolated JUE_HOME with minimal logs."""
    home = tmp_path / ".jue"
    home.mkdir()
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, jue_home):
    ...
    def test_returns_none_for_empty(self, jue_home):
        # Truncate agent.log to empty
        (jue_home / "logs" / "agent.log").write_text("")

        from hermes_cli.debug import _read_full_log
        assert _read_full_log("agent") is None

    def test_truncates_large_file(self, jue_home):
        """Files larger than max_bytes get tail-truncated."""
        from hermes_cli.debug import _read_full_log

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (jue_home / "logs" / "agent.log").write_text(big_content * 200)

        content = _read_full_log("agent", max_bytes=1024)
        assert content is not None
        assert "truncated" in content

    def test_unknown_log_returns_none(self, jue_home):
        from hermes_cli.debug import _read_full_log
        assert _read_full_log("nonexistent") is None

    def test_falls_back_to_rotated_file(self, jue_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from hermes_cli.debug import _read_full_log

        logs_dir = jue_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, jue_home):
    ...
    def test_falls_back_when_primary_empty(self, jue_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, jue_home):
        ...
        assert "--- jue dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, jue_home):
        from hermes_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, jue_home):
        from hermes_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, jue_home):
        from hermes_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_local_flag_prints_full_logs(self, jue_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, jue_home, capsys):
        ...
        with patch("hermes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- jue dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, jue_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, jue_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "jue debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, jue_home):
        from hermes_cli.debug import run_debug
...
class TestScheduleAutoDelete:
    ...
    ``~/.jue/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``jue debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, jue_home):
    ...
    def test_records_pending_to_json(self, jue_home):
    ...
    def test_skips_non_paste_rs_urls(self, jue_home):
    ...
    def test_merges_with_existing_pending(self, jue_home):
    ...
    def test_dedupes_same_url(self, jue_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, jue_home):
    ...
    def test_sweep_deletes_expired_entries(self, jue_home):
    ...
    def test_sweep_leaves_future_entries_alone(self, jue_home):
    ...
    def test_sweep_survives_network_failure(self, jue_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, jue_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, jue_home):
        from hermes_cli.debug import run_debug
    ...
    def test_run_debug_survives_sweep_failure(self, jue_home, capsys):
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, jue_home, capsys):
        from hermes_cli.debug import run_debug_share
    ...
    def test_share_shows_privacy_notice(self, jue_home, capsys):
        from hermes_cli.debug import run_debug_share
    ...
    def test_local_no_privacy_notice(self, jue_home, capsys):
        from hermes_cli.debug import run_debug_share
