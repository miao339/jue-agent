"""Tests for the ``jue hooks`` CLI subcommand."""
...
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("JUE_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("JUE_ACCEPT_HOOKS", raising=False)
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `jue hooks doctor` used to run every
