"""Tests for the `jue memory reset` CLI command.
...
- Profile-scoped reset (uses JUE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake JUE_HOME with memory files."""
    jue_home = tmp_path / ".jue"
    memories = jue_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("JUE_HOME", str(jue_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nJue repo is at ~/.jue/jue-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `jue memory reset` is run.
    """
    from jue_constants import get_jue_home, display_jue_home

    mem_dir = get_jue_home() / "memories"
    files_to_reset = []
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        jue_home = tmp_path / ".jue"
        (jue_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("JUE_HOME", str(jue_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's JUE_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        jue_home = tmp_path / ".jue"
        jue_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("JUE_HOME", str(jue_home))

        # The memories dir won't exist; get_jue_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
