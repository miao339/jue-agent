"""Tests for the Jue plugin system (hermes_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<jue_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<jue_home>/plugins/``; we derive
    ``<jue_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <JUE_HOME>/config.yaml.
        # Config is always read from JUE_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        jue_home_str = os.environ.get("JUE_HOME")
        if jue_home_str:
            jue_home = Path(jue_home_str)
        else:
            jue_home = base.parent
        jue_home.mkdir(parents=True, exist_ok=True)
        cfg_path = jue_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.jue/plugins/ are discovered."""
        plugins_dir = tmp_path / "jue_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("JUE_HOME", str(tmp_path / "jue_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JUE_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".jue" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under jue_plugins.<name>."""
        plugins_dir = tmp_path / "jue_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("JUE_HOME", str(tmp_path / "jue_test"))

        # Clean up any prior namespace module
        sys.modules.pop("jue_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "jue_plugins.ns_plugin" in sys.modules
...
class TestPluginCommands:
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "jue_test" / "plugins"
        # Set JUE_HOME BEFORE _make_plugin_dir so auto-enable targets
