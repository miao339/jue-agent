...
from hermes_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_jue_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and JUE_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.jue/profiles)
    * JUE_HOME  -> tmp_path/.jue  (so get_jue_home() agrees)
    * Creates the bare-minimum ~/.jue directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".jue"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("JUE_HOME", str(default_home))
    return tmp_path
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".jue" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_jue_home_returns_default(self, profile_env):
        # JUE_HOME points to tmp_path/.jue which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".jue" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom JUE_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("JUE_HOME", str(custom))
        # With Docker-aware roots, a custom JUE_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("jue")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".jue" / "profiles" / "oldname"
        ...
        assert new_dir == tmp_path / ".jue" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("jue-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".jue_history",
        ...
        excluded_prefixes = [
            "default/jue-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.jue_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_jue_profiles_function(self):
        script = generate_bash_completion()
        assert "_jue_profiles" in script

    def test_zsh_completion_has_jue_function(self):
        script = generate_zsh_completion()
        assert "_jue" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultJueHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_jue_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".jue" / "profiles"

    def test_default_jue_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_jue_home()
        assert home == tmp_path / ".jue"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (JUE_HOME outside ~/.jue), profiles go under JUE_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JUE_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_jue_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_jue_home() returns JUE_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JUE_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (JUE_HOME under ~/.jue), profiles root is still ~/.jue/profiles."""
        native = tmp_path / ".jue"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under JUE_HOME."""
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under JUE_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JUE_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JUE_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".jue" / "profiles" / "mybot"
        assert result == expected
