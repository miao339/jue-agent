...
class TestIsSkillDisabled:
    ...
    @patch("hermes_cli.config.load_config")
    @patch.dict("os.environ", {"JUE_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("JUE_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """JUE_SESSION_PLATFORM should be used when JUE_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_jue_platform_takes_precedence(self, tmp_path, monkeypatch):
        """JUE_PLATFORM should win over JUE_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("JUE_HOME", str(tmp_path))
        monkeypatch.setenv("JUE_PLATFORM", "telegram")
        monkeypatch.setenv("JUE_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JUE_HOME", str(tmp_path))
        monkeypatch.setenv("JUE_PLATFORM", "telegram")
        monkeypatch.setenv("JUE_SESSION_PLATFORM", "telegram")
