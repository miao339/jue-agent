...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "jue"
        assert config.workspace_id == "jue"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.jue/honcho.json
        monkeypatch.setenv("JUE_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"jue": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="jue-agent"
        ):
            result = config.resolve_session_name("/home/user/jue-agent/subdir")
        assert result == "jue-agent"
...
class TestResolveConfigPath:
    def test_prefers_jue_home_when_exists(self, tmp_path):
        jue_home = tmp_path / "jue"
        jue_home.mkdir()
        local_cfg = jue_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        jue_home = tmp_path / "jue"
        jue_home.mkdir()
        # No honcho.json in JUE_HOME — also isolate ~/.jue so
    ...
    def test_falls_back_to_global_without_jue_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("JUE_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_jue(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("JUE_HONCHO_HOST", None)
            os.environ.pop("JUE_HOME", None)
            assert resolve_active_host() == "jue"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"JUE_HONCHO_HOST": "jue.coder"}):
            assert resolve_active_host() == "jue.coder"
    ...
    def test_default_profile_returns_jue(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JUE_HONCHO_HOST", None)
            with patch("hermes_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "jue"

    def test_custom_profile_returns_jue(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JUE_HONCHO_HOST", None)
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jue.coder")
        assert config.host == "jue.coder"
        assert config.workspace_id == "jue"  # shared workspace
        assert config.ai_peer == "jue.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jue")
        assert config.host == "jue"
        assert config.workspace_id == "jue"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="jue.coder", config_path=config_file,
        )
        assert config.host == "jue.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "jue.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="jue.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "jue.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jue": {"enabled": True, "aiPeer": "jue"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jue": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_jue_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="jue",
    ...
    def test_jue_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="jue",
