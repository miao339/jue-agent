...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate JUE_HOME for each test.

    The global hermetic fixture already redirects JUE_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    JUE_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "jue_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "jue_plugins" not in sys.modules:
        ns = types.ModuleType("jue_plugins")
        ns.__path__ = []
        sys.modules["jue_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "jue_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["jue_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_jue_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_jue_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_jue_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/jue-abc/x.log")) is True
...
class TestBundledDiscovery:
    def _write_enabled_config(self, jue_home, names):
