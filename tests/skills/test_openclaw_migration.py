...
"""
    ...
    assert "Tyler Williams > Active Projects: Jue Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `jue cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # jue_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.jue/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "jue_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Jue prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Jue to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Jue config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "Jue should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Jue uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Jue remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "Jue prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Jue was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "Jue uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Jue said to ask Jue about Jue settings"
