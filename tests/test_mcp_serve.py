"""
Tests for mcp_serve — Jue MCP server.
...
def _isolate_jue_home(tmp_path, monkeypatch):
    """Redirect JUE_HOME to a temp directory."""
    monkeypatch.setenv("JUE_HOME", str(tmp_path))
    try:
        import jue_constants
        monkeypatch.setattr(jue_constants, "get_jue_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking jue_state schema."""
