...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("JUE_HOME", "/tmp/jue_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from JUE_HOME and verify it rewrites results."""
    import yaml

    jue_home = Path(os.environ["JUE_HOME"])
