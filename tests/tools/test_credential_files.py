...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        jue_home.mkdir()
        (jue_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"JUE_HOME": str(jue_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(jue_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.jue/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        (jue_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        (jue_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.jue/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        jue_home = tmp_path / ".jue"
        (jue_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"JUE_HOME": str(jue_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.jue")

        assert mounts[0]["container_path"] == "/home/user/.jue/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.jue/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.jue/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape JUE_HOME."""
        monkeypatch.setenv("JUE_HOME", str(tmp_path / ".jue"))
        (tmp_path / ".jue").mkdir()

        # Create a sensitive file one level above jue_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside jue_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside JUE_HOME must still be registered."""
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        monkeypatch.setenv("JUE_HOME", str(jue_home))
        (jue_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_jue_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of JUE_HOME must be allowed."""
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        subdir = jue_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside JUE_HOME pointing outside must be rejected."""
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        monkeypatch.setenv("JUE_HOME", str(jue_home))

        # Create a sensitive file outside jue_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside jue_home pointing outside
        symlink = jue_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes JUE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, jue_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape JUE_HOME."""
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        monkeypatch.setenv("JUE_HOME", str(jue_home))
        ...
        sensitive.write_text("{}")
        self._write_config(jue_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(jue_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside JUE_HOME via config must still mount."""
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        monkeypatch.setenv("JUE_HOME", str(jue_home))

        (jue_home / "oauth.json").write_text("{}")
        self._write_config(jue_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        (jue_home / "cache" / "documents").mkdir(parents=True)
        (jue_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("JUE_HOME", str(jue_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.jue/cache/documents" in paths
        assert "/root/.jue/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.jue/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        jue_home = tmp_path / ".jue"
        jue_home.mkdir()
        # Use legacy dir name — get_jue_dir prefers old if it exists
        (jue_home / "document_cache").mkdir()
        (jue_home / "image_cache").mkdir()
        monkeypatch.setenv("JUE_HOME", str(jue_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(jue_home / "document_cache") in host_paths
        assert str(jue_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.jue/cache/documents" in container_paths
        assert "/root/.jue/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        jue_home = tmp_path / ".jue"
        doc_dir = jue_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        jue_home = tmp_path / ".jue"
        ss_dir = jue_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.jue/cache/screenshots/session_abc/screen1.png"
