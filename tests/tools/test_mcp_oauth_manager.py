...
def test_jue_provider_subclass_exists():
    """JueMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _JUE_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _JUE_PROVIDER_CLS is not None
    assert issubclass(_JUE_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_jue_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns JueMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _JUE_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("JUE_HOME", str(tmp_path))
    ...
    assert _JUE_PROVIDER_CLS is not None
    assert isinstance(provider, _JUE_PROVIDER_CLS)
    assert provider._jue_server_name == "srv"
