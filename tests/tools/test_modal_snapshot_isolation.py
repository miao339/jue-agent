...
def _restore_tool_modules():
    original_jue_home = os.environ.get("JUE_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_jue_home is None:
            os.environ.pop("JUE_HOME", None)
        else:
            os.environ["JUE_HOME"] = original_jue_home
...
def _install_modal_test_modules(
    ...
    *,
    ...
    os.environ["JUE_HOME"] = str(jue_home)
    sys.modules["hermes_cli.config"] = types.SimpleNamespace(
        get_jue_home=lambda: jue_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="jue-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": jue_home / "modal_snapshots.json",
