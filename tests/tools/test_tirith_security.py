...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._jue_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_jue_bin(self):
        """After _INSTALL_FAILED, manual install in JUE_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        jue_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(jue_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(jue_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._jue_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == jue_bin
            assert _tirith_mod._resolved_path == jue_bin
            mock_clear.assert_called_once()
...
class TestJueHomeIsolation:
    def test_jue_bin_dir_respects_jue_home(self):
        """_jue_bin_dir must use JUE_HOME, not hardcoded ~/.jue."""
        from tools.tirith_security import _jue_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"JUE_HOME": tmpdir}):
            result = _jue_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_jue_home(self):
        """_failure_marker_path must use JUE_HOME, not hardcoded ~/.jue."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"JUE_HOME": "/custom/jue"}):
            result = _failure_marker_path()
        assert result == "/custom/jue/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets JUE_HOME; verify it's active."""
        jue_home = os.getenv("JUE_HOME")
        assert jue_home is not None, "JUE_HOME should be set by conftest"
        assert "jue_test" in jue_home, "Should point to test temp dir"

    def test_get_jue_home_fallback(self):
        """Without JUE_HOME set, falls back to ~/.jue."""
        from tools.tirith_security import _get_jue_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove JUE_HOME entirely
            os.environ.pop("JUE_HOME", None)
            result = _get_jue_home()
        assert result == os.path.join(os.path.expanduser("~"), ".jue")
