"""Tests for --yolo (JUE_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When JUE_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("JUE_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With JUE_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("JUE_YOLO_MODE", "1")
        monkeypatch.setenv("JUE_INTERACTIVE", "1")
        monkeypatch.setenv("JUE_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """JUE_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("JUE_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for JUE_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("JUE_YOLO_MODE", "")
        monkeypatch.setenv("JUE_INTERACTIVE", "1")
        monkeypatch.setenv("JUE_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("JUE_YOLO_MODE") returns ""
