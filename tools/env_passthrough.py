...
def _is_jue_provider_credential(name: str) -> bool:
    """True if ``name`` is a Jue-managed provider credential (API key,
    token, or similar) per ``_JUE_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Jue API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _JUE_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Jue-managed provider credentials (from
    ``_JUE_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Jue-managed
    ...
    Non-Jue third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_jue_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Jue provider "
                "credential %r (blocked by _JUE_PROVIDER_ENV_BLOCKLIST). "
