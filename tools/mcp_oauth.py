...
This module provides the glue:
    - ``JueTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Jue Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses JUE_HOME so each profile gets its own OAuth tokens.
    Layout: ``JUE_HOME/mcp-tokens/``
    """
    return get_jue_home() / "mcp-tokens"
...
# ---------------------------------------------------------------------------
# JueTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class JueTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        JUE_HOME/mcp-tokens/<server_name>.json         -- tokens
        JUE_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Jue records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Jue.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Jue Agent")
    scope = cfg.get("scope")
