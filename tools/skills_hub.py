#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Jue Skills Hub.
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            jue_meta = metadata.get("jue", {})
            if isinstance(jue_meta, dict):
                tags = jue_meta.get("tags", [])
        if not tags:
...
def _load_jue_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for JUE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if JUE_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - JUE_INDEX_CACHE_FILE.stat().st_mtime
            if age < JUE_INDEX_TTL:
                return json.loads(JUE_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(JUE_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Jue index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Jue index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        JUE_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        JUE_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class JueIndexSource(SkillSource):
    """Skill source backed by the centralized Jue Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "jue-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "jue-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        JueIndexSource(auth=auth), # Centralized index (search + resolved install paths)
