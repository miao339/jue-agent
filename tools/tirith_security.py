...
it is automatically downloaded from GitHub releases to $JUE_HOME/bin/tirith.
...
def _get_jue_home() -> str:
    """Return the Jue home directory, respecting JUE_HOME env var."""
    return str(get_jue_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_jue_home(), ".tirith-install-failed")
...
def _jue_bin_dir() -> str:
    """Return $JUE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_jue_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $JUE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_jue_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $JUE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $JUE_HOME/bin/tirith
    ...
    jue_bin = os.path.join(_jue_bin_dir(), "tirith")
    if os.path.isfile(jue_bin) and os.access(jue_bin, os.X_OK):
        _resolved_path = jue_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return jue_bin
