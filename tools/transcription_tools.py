...
LOCAL_STT_COMMAND_ENV = "JUE_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "JUE_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set JUE_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="jue-local-stt-") as output_dir:
