"""Persistent slash-command worker — one JueCLI per TUI session.
...
def _run(cli: JueCLI, command: str) -> str:
...
def main():
    ...
    os.environ["JUE_SESSION_KEY"] = args.session_key
    os.environ["JUE_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = JueCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
