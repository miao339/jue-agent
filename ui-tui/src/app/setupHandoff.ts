import type { RunExternalProcess } from '@jue/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`jue ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching jue: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`jue ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
