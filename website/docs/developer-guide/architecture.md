...
sidebar_position: 1
title: "Architecture"
description: "Jue Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Jue Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ jue_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # JueCLI — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── jue_state.py           # SQLite session/state database with FTS5
├── jue_constants.py       # JUE_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `jue` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # jue skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # jue tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # jue gateway start/stop
│
...
```text
User input → JueCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .jue.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.jue/plugins/` (user), `.jue/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `jue plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-jue-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Jue as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`jue -p <name>`) gets its own JUE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
