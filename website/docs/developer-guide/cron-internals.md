...
description: "How Jue stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `jue cron` subcommands |
...
Jobs are stored in `~/.jue/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `jue cron` commands are run or during active CLI sessions.
...
```python
# ~/.jue/scripts/check_competitors.py
...
2. **Environment variable** — `JUE_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.jue/cron/output/` |
...
The `jue cron` CLI provides direct job management:

```bash
jue cron list                    # Show all jobs
jue cron create                  # Interactive job creation (alias: add)
jue cron edit <job_id>           # Edit job configuration
jue cron pause <job_id>          # Pause a running job
jue cron resume <job_id>         # Resume a paused job
jue cron run <job_id>            # Trigger immediate execution
jue cron remove <job_id>         # Delete a job
```
