...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Jue-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Jue Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Jue/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- JueAgentBaseEnv
    JueAgentBaseEnv <|-- TerminalTestEnv
    JueAgentBaseEnv <|-- JueSweEnv
    JueAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The jue-agent layer (`environments/jue_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls jue-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `JueAgentLoop` and scores the result
...
Your environment inherits from `JueAgentBaseEnv` and implements five methods:
...
`JueAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as jue-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any jue-agent tool |
...
parser = get_parser("jue")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `jue`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "jue-agent[yc-bench]"
...
```bash
python environments/jue_swe_env/jue_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"jue"`, `"qwen"`, `"mistral"`)
...
```python
from environments.jue_base_env import JueAgentBaseEnv, JueAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(JueAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(JueAgentBaseEnv):
    name = "my-env"
...
### JueAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which jue toolsets to enable |
...
| `tool_call_parser` | `str` | `"jue"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "jue-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "jue-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── jue_base_env.py          # Abstract base class (JueAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (JueAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── jue_parser.py        # Jue/ChatML <tool_call> format
...
├── jue_swe_env/             # SWE-bench training environment
│
