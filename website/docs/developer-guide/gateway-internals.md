...
The messaging gateway is the long-running process that connects Jue to 14+ external messaging platforms through a unified architecture.
...
|--------|-----------------|
| `~/.jue/.env` | API keys, bot tokens, platform credentials |
| `~/.jue/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.jue/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `jue gateway start` / `jue gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.jue/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `jue gateway stop` stops only the current profile's gateway. `jue gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
