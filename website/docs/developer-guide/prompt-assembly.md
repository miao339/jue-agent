...
description: "How Jue builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Jue deliberately separates:
...
1. agent identity — `SOUL.md` from `JUE_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.jue/SOUL.md)
You are Jue, an AI assistant with judgment grounded in ROOT_PARADIGM and SOUL.
...
`SOUL.md` lives at `~/.jue/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are Jue Agent, an intelligent AI assistant with judgment grounded in ROOT_PARADIGM and SOUL.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_jue_md(cwd_path)       # 1. .jue.md / JUE.md (walks to git root)
    ...
    # SOUL.md from JUE_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.jue.md`, `JUE.md` | CWD up to git root | Jue-native project config |
...
- **YAML frontmatter stripped** — `.jue.md` frontmatter is removed (reserved for future config overrides)
...
1. `.jue.md` / `JUE.md` (walks to git root)
