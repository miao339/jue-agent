...
description: "Run Jue Agent directly on an Android phone with Termux"
---

# Jue on Android with Termux

This is the tested path for running Jue Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Jue from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Jue now ships a Termux-aware installer path:
...
- links `jue` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `jue` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/jue" "$PREFIX/bin/jue"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `jue` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.jue/.env`.
...
### `jue doctor` says ripgrep or Node is missing
