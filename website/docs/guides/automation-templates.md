...
Copy-paste recipes for common automation patterns. Each template uses Jue's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`jue webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `jue webhook subscribe`) |
...
```bash
jue cron create "0 2 * * *" \
  "You are a project manager triaging the miao339/jue-agent GitHub repo.

1. Run: gh issue list --repo miao339/jue-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
jue webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
jue cron create "0 9 * * 1" \
  "Scan the miao339/jue-agent repo for documentation drift.

1. Run: gh pr list --repo miao339/jue-agent --state merged --json number,title,files,mergedAt --limit 30
...
```bash
jue cron create "0 6 * * *" \
  "Run a dependency security audit on the jue-agent project.

1. cd ~/.jue/jue-agent && source .venv/bin/activate
...
```bash
jue webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
jue webhook subscribe alert-triage \
...
```python title="~/.jue/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Jue-Monitor/1.0"})
...
jue cron create "every 30m" \
  ...
  --script ~/.jue/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
jue cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Jue Agent development." \
...
```bash
jue webhook subscribe github-issues \
  --events "issues" \
...
```bash
jue webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
jue webhook subscribe stripe-payments \
...
```bash
jue cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the jue-agent codebase.
