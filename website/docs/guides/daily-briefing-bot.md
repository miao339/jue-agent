...
2. **Jue spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.jue/cron/output/` and you can read them anytime.
:::
...
Jue will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Jue Agent
```
...
Jue will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Jue to delegate each topic to a sub-agent:
...
Jue will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
