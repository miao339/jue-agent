...
This tutorial walks you through setting up a Telegram bot powered by Jue Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Jue Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.jue/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Jue itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Jue Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_jue_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Jue Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u jue-gateway -f
...
jue gateway start
jue gateway stop
tail -f ~/.jue/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `jue gateway install` to update the plist.
:::
...
```bash
jue gateway stop && jue gateway start
```
...
jue pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.jue/.env`:
...
Control how much detail the bot shows when using tools. In `~/.jue/config.yaml`:
...
Customize how the bot communicates by editing `~/.jue/SOUL.md`:

For a full guide, see [Use SOUL.md with Jue](/docs/guides/use-soul-with-jue).
...
```bash
# From the CLI
jue cron list          # View all scheduled jobs
jue cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u jue-gateway` (Linux) or `~/.jue/logs/gateway.log` (macOS) |
| Cron job output | `~/.jue/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.jue/cron/jobs.json` |
| Pairing data | `~/.jue/pairing/` |
| Session history | `~/.jue/sessions/` |
