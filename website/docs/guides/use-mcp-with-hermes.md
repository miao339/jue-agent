---
sidebar_position: 6
title: "Use MCP with Jue"
description: "A practical guide to connecting MCP servers to Jue Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Jue

This guide shows how to actually use MCP with Jue Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Jue tool
- you want Jue to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Jue to internal APIs, databases, or company systems without modifying Jue core

Do not use MCP when:
- a built-in Jue tool already solves the job well
...
- Jue remains the agent
- MCP servers contribute tools
- Jue discovers those tools at startup or reload time
...
If you installed Jue with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Jue banner/status should show MCP integration when configured
- ask Jue what tools it has available
...
There are two categories of MCP-exposed functionality in Jue:
...
So Jue will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Jue to reason over a bounded workspace.
...
Now Jue can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Jue core.
...
Because Jue now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
