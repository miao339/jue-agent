---
sidebar_position: 8
title: "Use Voice Mode with Jue"
description: "A practical guide to setting up and using Jue voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Jue
...
- you want Jue sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Jue.
...
## Step 1: make sure normal Jue works first
...
Jue supports both local and cloud speech stacks.
...
### If you use `jue setup`

If you choose NeuTTS in the setup wizard, Jue checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Jue transcribes and responds
...
- asking Jue to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Jue loop.

## Tuning CLI behavior

### Silence threshold

If Jue starts/stops too aggressively, tune:
...
Jue stays a normal chat bot, but can speak replies.
...
- you want Jue to function like a portable research or ops assistant
...
Jue joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Jue detects speech boundaries
- transcripts are posted in the associated text channel
- Jue responds in text and audio
...
1. get text Jue working
2. install `jue-agent[voice]`
