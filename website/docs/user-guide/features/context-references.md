...
Type `@` followed by a reference to inject content directly into your message. Jue expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Jue env: `$JUE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$JUE_HOME/skills/.hub/`
