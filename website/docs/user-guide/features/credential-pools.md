...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Jue automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Jue auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
jue auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
jue auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (Claude Code subscription)
jue auth add anthropic --type oauth
...
Run `jue auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `jue auth` | Interactive pool management wizard |
| `jue auth list` | Show all pools and credentials |
| `jue auth list <provider>` | Show a specific provider's pool |
| `jue auth add <provider>` | Add a credential (prompts for type and key) |
| `jue auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `jue auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `jue auth remove <provider> <index>` | Remove credential by 1-based index |
| `jue auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `jue auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `jue model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via jue model:
jue auth list
# Shows:
...
jue auth add Together.ai --api-key sk-together-second-key
```
...
Jue automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Jue PKCE OAuth | `~/.jue/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `jue auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `jue auth add`) are never auto-pruned.
...
Pool state is stored in `~/.jue/auth.json` under the `credential_pool` key:
