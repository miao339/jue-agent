...
Schedule tasks to run automatically with natural language or cron expressions. Jue exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Jue disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
jue cron create "every 2h" "Check server status"
jue cron create "every 1h" "Summarize new feed items" --skill blogwatcher
jue cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Jue will use the unified `cronjob` tool internally.
...
```bash
jue cron edit <job_id> --schedule "every 4h"
jue cron edit <job_id> --prompt "Use the revised task"
jue cron edit <job_id> --skill blogwatcher --skill maps
jue cron edit <job_id> --add-skill maps
jue cron edit <job_id> --remove-skill blogwatcher
jue cron edit <job_id> --clear-skills
```
...
```bash
jue cron list
jue cron pause <job_id>
jue cron resume <job_id>
jue cron run <job_id>
jue cron remove <job_id>
jue cron status
jue cron tick
```
...
sudo jue gateway install --system   # Linux: boot-time system service for servers
jue gateway             # Or run in foreground
...
1. loads jobs from `~/.jue/cron/jobs.json`
...
A file lock at `~/.jue/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.jue/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.jue/cron/output/`), but no message is sent to the delivery target.
...
Or set the `JUE_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Jue skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.jue/cron/jobs.json`. Output from job runs is saved to `~/.jue/cron/output/{job_id}/{timestamp}.md`.
