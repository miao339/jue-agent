...
Jue Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Jue can automatically switch to a backup provider:model pair mid-session without losing your conversation.

### Configuration

Add a `fallback_model` section to `~/.jue/config.yaml`:
...
| Nous Portal | `nous` | `jue auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `jue model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `jue model` (Google OAuth; optional: `JUE_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `jue model` (Qwen Portal OAuth; optional: `JUE_QWEN_BASE_URL`) |
...
Jue uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), Jue tries providers in order until one works:
...
If the resolved provider fails at call time, Jue also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, Jue also supports:
...
| `"codex"` | Force Codex OAuth | `jue model` → Codex |
...
`base_url` takes precedence over `provider`. Jue uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Jue drops middle conversation turns without generating a summary rather than failing the session.
