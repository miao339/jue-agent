...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Jue's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Jue instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
jue memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$JUE_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `jue` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Jue syncs them back at session init.
...
```bash
jue honcho status          # Connection status, config, and key settings
jue honcho setup           # Interactive setup wizard
jue honcho strategy        # Show or set session strategy
jue honcho peer            # Update peer names for multi-agent setups
jue honcho mode            # Show or set recall mode
jue honcho tokens          # Show or set context token budget
jue honcho identity        # Show Honcho peer identity
jue honcho sync            # Sync host blocks for all profiles
jue honcho enable          # Enable Honcho
jue honcho disable         # Disable Honcho
```

## Migrating from `jue honcho`

If you previously used the standalone `jue honcho setup`:
...
No re-login or re-setup needed. Run `jue memory setup` and select "honcho" — the wizard detects your existing config.
