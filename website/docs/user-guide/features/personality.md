...
If the file is empty, whitespace-only, or cannot be read, Jue falls back to a built-in default identity. This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
