...
description: "Overview of Jue Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Jue ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `jue model` to enable it, or configure individual tools with `jue tools`.
:::
...
jue chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `jue-cli`, `jue-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
jue config set terminal.backend singularity
jue config set terminal.singularity_image ~/python.sif
```
...
modal setup
jue config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.jue/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.jue/.env`.
:::
