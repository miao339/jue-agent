...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Jue Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Jue Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Jue](/docs/guides/use-voice-mode-with-jue).

## Prerequisites

Before using voice features, make sure you have:

1. **Jue Agent installed** — `pip install jue-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `jue model` or set your preferred provider credentials in `~/.jue/.env`
3. **A working base setup** — run `jue` to verify the agent responds to text before enabling voice

:::tip
The `~/.jue/` directory and default `config.yaml` are created automatically the first time you run `jue`. You only need to create `~/.jue/.env` manually for API keys.
:::
...
pip install "jue-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "jue-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "jue-agent[all]"
```
...
1. Start the CLI with `jue` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.jue/config.yaml` (default: `ctrl+b`).
:::
...
```bash
jue gateway        # Start the gateway (connects to configured platforms)
jue gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@juebyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.jue/.env`:
```bash
...
```bash
jue gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.jue/.env`
...
- Review gateway logs: `tail -f ~/.jue/logs/gateway.log`
