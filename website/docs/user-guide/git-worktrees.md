...
title: "Git Worktrees"
description: "Run multiple Jue agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Jue Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Jue so each session has a clean, isolated working directory.

## Why Use Worktrees with Jue?

Jue treats the **current working directory** as the project root:

- CLI: the directory where you run `jue` or `jue chat`
...
git worktree add ../repo-feature feature/jue-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/jue-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Jue there:

```bash
cd ../repo-feature

# Start Jue in the worktree
jue
```
...
git worktree add ../repo-experiment-a feature/jue-a
git worktree add ../repo-experiment-b feature/jue-b
```
...
- Works on its own branch (`feature/jue-a` vs `feature/jue-b`).
...
- Jue checkpoint data under `~/.jue/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Jue experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/jue-checkpoints-docs`, `feature/jue-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Jue from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `jue -w` (Automatic Worktree Mode)

Jue has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `jue/jue-<hash>`).
...
```bash
jue -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `jue -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Jue session its own clean checkout.
