...
Connect Jue to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Jue and the BlueBubbles server
...
Or set environment variables directly in `~/.jue/.env`:
...
When someone messages your iMessage, Jue automatically sends them a pairing code. Approve it with:
```bash
jue pairing approve bluebubbles <CODE>
```
Use `jue pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.jue/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.jue/.env`):
```bash
...
Jue will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Jue
Jue → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Jue sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Jue resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `jue logs gateway` for webhook errors (or `jue logs -f` to follow in real-time)
