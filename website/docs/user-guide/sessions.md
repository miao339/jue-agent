...
Jue Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.jue/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.jue/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`jue` or `jue chat`) |
...
jue -c "my project"   # → resumes "my project #3"
```
...
jue --resume 20250305_091523_a1b2c3d4
jue -r 20250305_091523_a1b2c3d4

# Resume by title
jue --resume "refactoring auth"

# Or with the chat subcommand
jue chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `jue sessions list`.

### Conversation Recap on Resume

When you resume a session, Jue displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Jue session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.jue/config.yaml`:
...
Jue automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `jue sessions list` or `jue sessions browse`.
...
```bash
jue sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Jue creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`jue -c "my project"`), it automatically picks the most recent session in the lineage.
...
Jue provides a full set of session management commands via `jue sessions`:
...
# Filter by platform
jue sessions list --source telegram

# Show more sessions
jue sessions list --limit 50
```
...
jue sessions export backup.jsonl

# Export sessions from a specific platform
jue sessions export telegram-history.jsonl --source telegram

# Export a single session
jue sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
jue sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
jue sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
jue sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
jue sessions prune --older-than 30

# Only prune sessions from a specific platform
jue sessions prune --source telegram --older-than 60

# Skip confirmation
jue sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`jue insights`](/docs/reference/cli-commands#jue-insights).
...
When Jue cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Jue uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Jue in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.jue/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.jue/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.jue/sessions/sessions.json` | Maps session keys to active session IDs |
...
jue sessions delete <session_id>
